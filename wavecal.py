...
def arcgauss(r, p):
    ...
    pars = l.guess(arcdata[r, p-10:p+10], x=x[p-10:p+10])
    pars.update(l.make_params())
    pars['l_center'].set(p, min=p-10, max=p+10)
    pars['l_sigma'].set(0.5, max=np.sqrt(3))
    pars['l_amplitude'].set(50, min=0)
    mod = l
    plt.plot(x[p-10:p+10], arcdata[r, p-10:p+10], label='data')
    out = mod.fit(arcdata[r, p-10:p+10], pars, x=x[p-10:p+10])
    comps = out.eval_components(x=x[p-10:p+10])
    print(out.fit_report(min_correl=0.5))
    plt.plot(x[p-10:p+10], out.best_fit, 'r-', label='best fit')
    plt.plot(x[p-10:p+10], comps['l_'], 'b--', label='line')
    plt.legend()
    plt.show()
...
fepxsb = np.array([372., 379., 1121., 1307., 1370., 1384., 1464., 1470., 1609., 1638., 1652., 1705., 1784., 1818., 2204., 2285., 2439., 2554., 2718., 2739., 2808., 3084.])
