...
#Call in format 'python target_altitude.py year month day obs_long obs_lat RA(HH:MM:SS) Dec(DD:MM:SS)'
...
h_ra = sys.argv[6].split(':')
h_dec = sys.argv[7].split(':')
...
plt.ylim([0, 90])
plt.title('Altitude of object at RA=%s Dec=%s at location Lat=%s Long=%s on date %s/%s/%s' %(sys.argv[6], sys.argv[7], sys.argv[5], sys.argv[4], sys.argv[1], sys.argv[2], sys.argv[3]))
